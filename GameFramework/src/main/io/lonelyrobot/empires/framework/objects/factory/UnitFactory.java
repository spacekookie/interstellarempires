...
import io.lonelyrobot.empires.framework.objects.OrbitalStructure;
...
public class UnitFactory {
	...
	public Ship requisitionUnit(ShipType type, Player Claim) {
		...
		Ship ship = new Ship(type, null, null, parent.getPosition());
		return ship;
	}

	public Ship requisitionUnit(Type type) {
		logger
				.info("Yea...this doesn't do a whole lot yet. Come back later. Or consult with your System Administrator. Wink Wink!");
		return null;
	}
	...
	public Fleet requisitionFleet(Set<ShipType> types) {
		...
		for (ShipType type : types) {
			requested.add(new Ship(type, null, null, parent.getPosition()));
		}
