...
public class LoginStatusOverlay extends R2Overlay {
  ...
  public void build() {
	...
	TextureRegionDrawable textureBar = new TextureRegionDrawable(new TextureRegion(new Texture(
		Gdx.files.internal("barGreen_horizontalMid.png"))));
  ...
  private void contactMothership(Server server) {
	...
	// prog = ConStep.FINISH;
	// else {
	// String[] con = new String[] { "Handshaking and verification with the Server failed!",
	// "Please try again later or check with the server provider",
	// "for more information if the problem persists.", "" };
	// final R2Dialogue m = new R2Dialogue("Handshake Failed", con, Assets.R2_UI_SKIN, DialogueType.ERROR);
	// m.button("OK", new ClickListener() {
	// public void touchUp(InputEvent arg0, float arg1, float arg2, int arg3, int arg4) {
	// m.remove();
	// GameCore.getInstance().removeOverlay();
