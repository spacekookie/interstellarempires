...
package io.lonelyrobot.empires.client.networking;
...
import io.lonelyrobot.empires.framework.network.ConnectionPackage;
import io.lonelyrobot.empires.framework.network.Host;
import io.lonelyrobot.empires.framework.network.NetworkHelper;
import io.lonelyrobot.empires.framework.network.Host.IP;

/**
 * Main connection handler to hold a Client-Server communication.
 * 
 * Pushes data onto the game screens to be dispayed and exchanges certificates
 * with server.
 * 
...
public class ConnectionHandler {
		...
		client.addListener(new Listener() {
			public void received(Connection connection, Object obj) {
				if (obj instanceof ConnectionPackage) {
					...
					// Checks that the server added information to the package
					// and it is thus valid!
					// if (((ConnectionPackage) obj).getServerNameResponse() !=
					// null
	...
	public boolean connectInit(Server server) {
		...
		// try {
		// System.out.println("Attempting to connect to: " + server.getUrl() + "
		// " + server.getPortTCP() + "/"
		// + server.getPortUDP());
		// client.connect(timeoutTimeInMillis, server.getUrl(),
		// server.getPortTCP(), server.getPortUDP());
		// return true;
		// } catch (IOException e) {
		// logger.error("Server " + server.toCompactString() + " is currently
		// not available!");
		// return false;
		// }
		return false;
