...
public class HexMapScreen implements Screen {
	...
	public void show() {
		...
		for (int mx = 0; mx < 3; mx++) {
			for (int my = 0; my < 3; my++) {
				SolarSystem sys = new SolarSystem(new GalaxyPosition(mx, my),
						new Player("Julie"), null, null, null, new Star(
	...
	public void render(float delta) {
		...
		/** Sets up map view */
		Gdx.gl.glViewport(mapDim.getKey().x, mapDim.getKey().y,
				mapDim.getValue().x, mapDim.getValue().y);
		mapCam.update();
		...
		shapeRenderer.rect(mapDim.getKey().x, mapDim.getKey().y,
				mapDim.getValue().x, mapDim.getValue().y);
		shapeRenderer.end();
	...
	private void setupProfileDialoge() {
		...
		profile_bottomButton.setSize(BaseSettings.OLD_WIDTH / 2,
				BaseSettings.OLD_HEIGHT / 2);
	...
	private void setupListeners() {
		...
		profile.addListener(new ClickListener() {
			...
			public void touchUp(InputEvent event, float x, float y,
				...
				profileDialog.setPosition((BaseSettings.OLD_WIDTH / 2)
						- (BaseSettings.OLD_WIDTH / 4),
						(BaseSettings.OLD_HEIGHT / 2)
	...
	private void setupInfoLabelBottom() {
		...
		systemSelector = new Label("Currently selected Solar System: "
				+ "545-101: KateTheAwesome: Red Giant", Resources.UI_SKIN);
		selectorTable.setPosition(mapDim.getKey().x + 255,
				mapDim.getKey().y - 10);
